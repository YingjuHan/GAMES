...
#pragma omp parallel for
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            Float3 pre_local_position = world_to_local(world_position, Float3::EType::Point);
            Float3 pre_world_position = pre_local_to_world(pre_local_position, Float3::EType::Point);
            Float3 pre_screen_position = pre_World_To_Screen(pre_world_position, Float3::EType::Point);
        ...
        for (int x = 0; x < width; x++) {
            ...
            for (int i = x_start; i <= x_end; i++) {
                for (int j = y_start; j <= y_end; j++) {
                    Float3 position = frameInfo.m_position(i, j);
                    Float3 normal = frameInfo.m_normal(i, j);
                    Float3 color = frameInfo.m_beauty(i, j);

                    float d_position = SqrDistance(center_position, position) / (2.0f * m_sigmaCoord * m_sigmaCoord);
                    float d_normal = SafeAcos(Dot(center_normal, normal)) * SafeAcos(Dot(center_normal, normal)) / (2.0f * m_sigmaNormal * m_sigmaNormal);
                    float d_color = SqrDistance(center_color, color) / (2.0f * m_sigmaColor * m_sigmaColor);
                    float d_plane = .0f;
                    if (d_position > 0.f) {
                        d_plane = Dot(center_normal, Normalize(position - center_position));
                    }
                    d_plane *= d_plane;
