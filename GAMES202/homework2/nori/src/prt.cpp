    ...
    virtual void preprocess(const Scene* scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                Eigen::Array3d d = sh::ToVector(phi, theta); // 将球坐标转换为方向向量
                const auto wi = Vector3f(d.x(), d.y(), d.z());
                // Edit Start
                double H = wi.normalized().dot(n.normalized()); // 方向与法向量的点积
                // Edit End
                if (m_Type == Type::Unshadowed) // 无阴影传输项
                {
                    // TODO: here you need to calculate unshadowed transport term of a given direction
                    // TODO: 此处你需要计算给定方向下的unshadowed传输项球谐函数值
                    return H > 0.0 ? H : 0;
                }
                else // 有阴影传输项，通过光线追踪（rayIntersect）判断可见性，仅返回可见方向的H
                {
            ...
            {
                // Edit Start
                m_TransportSHCoeffs.col(i).coeffRef(j) = (*shCoeff)[j] / M_PI; // 除以π归一化
                // Edit End
            }
        }
        if (m_Type == Type::Interreflection)
        {
            ...
            for (int i = 0; i < mesh->getVertexCount(); i++)
            {
                ...
                auto indirectCoeffs = computeInterreflectionSH(&m_TransportSHCoeffs, v, n, scene, 1); // 计算多次反射的间接光照
                for (int j = 0; j < SHCoeffLength; j++)
                {
                    m_TransportSHCoeffs.col(i).coeffRef(j) += (*indirectCoeffs)[j]; // 叠加间接光照
                }
